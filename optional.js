...
async function getWeather(city) {
    try {
        const apikey = "26871e3dc9493a0fbcee6f7f70eb7bc1";
        ...
        if (temp < 0 || temp > 35) {
            throw new ExtremeTemperatureError("Extreme temperature detected");
        }

        console.log(`Weather in ${result.name}: ${temp}°C`);
        return "Weather OK";

    } catch (error) {
        console.log(error.name + ":", error.message);
        throw error; // important for chaining
    }
}
...
async function runDashboard(city) {
    try {
        await getWeather(city);

        console.log("Sending report emails...");

        const tasks = emails.map(sendEmail);
